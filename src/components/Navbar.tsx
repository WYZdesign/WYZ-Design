...
import { FiSun, FiMoon, FiSearch, FiZap, FiShare2, FiGift, FiCalendar, FiUser } from "react-icons/fi";
...
export default function Navbar() {
  ...
  return (
    <>
      <nav aria-label="Main navigation" className={`fixed top-0 left-0 right-0 z-50 transition-all duration-700 ${visible ? "opacity-100" : "opacity-0 -translate-y-2 pointer-events-none"}`}>
                      ...
                      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 8 }}
                        ...
                        <div className="relative z-10 py-2">
                          <div className="px-4 py-3 border-b border-white/10 flex items-center gap-3">
                            <div className="w-9 h-9 rounded-full overflow-hidden border-2 border-white/30 flex items-center justify-center bg-white/10 shrink-0">
                              {session.user.image ? (
                                <Image src={session.user.image} alt={session.user.name || "Account"} width={36} height={36} className="w-full h-full object-cover" />
                              ) : (
                                <span className="w-full h-full rounded-full bg-[#DF3131] text-white flex items-center justify-center font-heading font-bold text-[13px]">
                                  {session.user.name?.[0]?.toUpperCase() || "U"}
                                </span>
                              )}
                            </div>
                          ...
                          {/* Quick Links */}
                          <div className="px-4 py-3 border-b border-white/10">
                            <p className="text-xs font-bold tracking-[0.15em] uppercase text-white/50 mb-2">Quick Links</p>
                            <Link href="/booking" onClick={() => setProfileOpen(false)}
                              className="block px-3 py-2 text-sm text-white/70 hover:text-white hover:bg-white/10 transition-all flex items-center gap-2">
                          ...
                          <Link href="/account/my-account" onClick={() => setProfileOpen(false)}
                            className="block px-4 py-2 text-sm text-white/70 hover:text-white hover:bg-white/10 transition-all flex items-center gap-2">
                            <FiUser className="w-3 h-3 text-[#DF3131]" />
                            My Account
                          </Link>

    ...
    <section className="py-20 bg-white">
      <div className="max-w-[115rem] mx-auto px-6 lg:px-12">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-10">
          <h2 className="text-3xl sm:text-4xl font-heading font-bold text-[#333333] tracking-[0.1em]">Pricing Plans</h2>
          <p className="text-[#666665] mt-2 text-[15px]">Affordable Plans for Any Budget</p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 mt-12">
          {PLANS.map((p, i) => (
            <motion.div key={p.name} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}
              className={`relative border bg-white flex flex-col ${p.popular ? "border-[#DF3131] shadow-md" : "border-[#E2E2E2]"}`}>
              ...
              <div className="p-6 text-center border-b border-[#E2E2E2]">
                <h3 className="text-lg font-heading font-bold text-[#333333]">{p.name}</h3>
                <div className="mt-3 text-[#333333]"><span className="text-3xl font-heading font-black">{p.price}</span><span className="text-[#8F8F8F] text-sm">/{p.period.toLowerCase().replace("every ","")}</span></div>
                <span className="inline-block mt-2 text-[14px] text-[#DF3131] font-semibold bg-[#DF3131]/5 px-2 py-0.5">{p.value}</span>
                <p className="text-[14px] text-[#8F8F8F] mt-1">Valid for 3 months</p>
              </div>
              <div className="p-6 flex-1">
                <ul className="space-y-2.5">
                  {p.features.map((f) => (
                    <li key={f} className="flex items-start gap-2 text-[13px] text-[#666665]"><FiCheck className="w-4 h-4 text-[#DF3131] mt-0.5 shrink-0" />{f}</li>
                  ))}
                </ul>
              </div>
        ...
        <p className="mt-8 text-[15px] text-[#8F8F8F] text-center max-w-3xl mx-auto leading-relaxed">

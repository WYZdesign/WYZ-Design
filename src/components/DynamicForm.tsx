...
export default function DynamicForm({
  ...
  const maxPage = paginated ? Math.max(...fields.map(f => f.page ?? 0)) : 0;
  const visibleFields = paginated ? fields.filter(f => (f.page ?? 0) === page) : fields;
  ...
  const renderField = (field: FormField) => {
    const inputClasses = "w-full px-4 py-3 border border-[#E2E2E2] dark:border-[#555] text-[14px] placeholder:text-[#8F8F8F] text-[#333] dark:text-white dark:bg-[#2b2b2e] focus:border-[#DF3131] focus:ring-1 focus:ring-[#DF3131]/20 outline-none transition-all";
    const wrapperClass = field.halfWidth ? "" : "sm:col-span-2";

    return (
      <div key={`${field.name}-${field.label}`} className={wrapperClass}>
      ...
      {paginated && (
        <div className="flex items-center justify-center gap-2 mb-6">
          {Array.from({ length: maxPage + 1 }, (_, i) => (
            <div key={i} className={`w-2 h-2 rounded-full transition-all ${i === page ? "bg-[#DF3131] scale-125" : i < page ? "bg-[#DF3131]/50" : "bg-gray-300"}`} />
          ))}
        </div>
      )}
        ...
        <div className="flex gap-3 mt-6">
          {page > 0 && (
            <button type="button" onClick={prevPage}
              className="flex-1 py-3.5 border border-[#E2E2E2] dark:border-[#555] text-[#333] dark:text-[#e0e0e0] font-heading font-bold text-sm tracking-[0.1em] uppercase hover:bg-gray-50 dark:hover:bg-[#333] transition-all">
              BACK
            </button>
          )}
          {isLastPage ? (
            <button type="submit" disabled={loading}
              className="flex-1 py-3.5 bg-[#DF3131] text-white font-heading font-bold text-sm tracking-[0.1em] uppercase hover:bg-[#B82020] transition-all disabled:opacity-50">
              {loading ? "SENDING..." : submitLabel}
            </button>
          ) : (
            <button type="button" onClick={nextPage}
              className="flex-1 py-3.5 bg-[#DF3131] text-white font-heading font-bold text-sm tracking-[0.1em] uppercase hover:bg-[#B82020] transition-all">
              NEXT
            </button>
          )}
        </div>

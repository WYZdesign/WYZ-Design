...
export default function ConsultationCalendar() {
  ...
  const [topic, setTopic] = useState("branding");
  ...
  const isWeekend = (day: number) => new Date(viewYear, viewMonth, day).getDay() === 0;
    ...
    try {
      const res = await fetch("/api/forms", {
        ...
        if (!res.ok || !data.success) {
          logger.warn("consultation-page", `Form submit failed: ${data.error || res.status}`);
          toast.error(data.error || "Scheduling failed. Please try again.");
          return;
        }
...
<main className="min-h-screen bg-white dark:bg-[#1C1C1E] pb-20">
          <div className="max-w-lg mx-auto px-6 text-center">
            ...
            <p className="text-lg text-[#333333] dark:text-white mb-6">{selectedTime}, 30 min</p>
            <p className="text-sm text-[#666665] dark:text-white/70 mb-8">A calendar invite has been sent to {email}. We will send a video call link 1 hour before the session.</p>
            <Link href="/" className="bg-[#333333] text-white px-8 py-3 font-heading font-bold tracking-[0.15em] uppercase hover:bg-[#DF3131] transition-colors inline-block">Back to Home</Link>
          </div>
        </main>
</>
    );
                      ...
                      return (
                        <button
                          ...
                          className={`py-2 text-sm font-semibold border transition-colors ${
                            selectedTime === time ? "border-[#DF3131] bg-[#DF3131] text-white"                               : "border-gray-200 dark:border-[#444] text-[#333333] dark:text-[#e0e0e0] hover:border-[#DF3131]"
                          }`}
                        >
                          {time}
                        </button>
